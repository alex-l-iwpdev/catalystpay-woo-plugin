        ...
        class WC_Gateway_Catalystpay extends WC_Payment_Gateway_CC {
            ...
            public function form() {
                ...
                if ( is_checkout() && empty( $wp->query_vars['order-pay'] ) && ! isset( $wp->query_vars['order-received'] ) ) {
                    ...
                    <div class="modal catalystpay-wrapper" tabindex="-1" role="dialog" id="ccModal">
                        ...
                        <form action="#" class="paymentWidgets" data-brands="VISA MASTER AMEX<?php echo $brands; ?>" id="cc-form"></form>
                        ...
                        <div class="footer-images">
                            <img src="<?php echo esc_url( $plugin_dir_url . 'assets/pci-dss.png' ); ?>" class="footer-img">
                            <img src="<?php echo esc_url( $plugin_dir_url . 'assets/secure-ssl.jpg' ); ?>" class="footer-img">
                            <img src="<?php echo esc_url( $plugin_dir_url . 'assets/verified-by-visa.jpg' ); ?>" class="footer-img">
                            <img src="<?php echo esc_url( $plugin_dir_url . 'assets/mastercard.jpg' ); ?>" class="footer-img">
                        </div>
                    </div>
                    </div>
                    <?php

                    wp_enqueue_style( 'catalystpay_style_modal', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-modal/0.9.2/jquery.modal.min.css');
                    wp_enqueue_style( 'catalystpay_style', CATALYSTPAY_PLUGIN_DIR . '/assets/css/style.css');
                    if (!empty($brands)) {
                        wp_enqueue_style( 'catalystpay_style_apm', CATALYSTPAY_PLUGIN_DIR . '/assets/css/apm.css');
                    }
                    ...
                    wp_enqueue_script('wpwl_catalystpay_script',  CATALYSTPAY_PLUGIN_DIR . '/assets/js/script.js', ['jquery'], false, true);
                }
            }
            ...
            if (!$is_subscription_product) {
                ...
                foreach ($order->get_items() as $product) {
                    ...
                    $data['cart.items['.$key.'].originalPrice'] = number_format((float) $product->get_product()->get_price(), 2, '.', '');
                    ...
                    if ($catalystpay->hasErrors()) {
                        ...
                        if (!empty($result['result']['code'])) {
                            if (preg_match('/^(000.000.|000.100.1|000.[36]|000.400.[1][12]0)/', $result['result']['code'])) {
                                $status = 'success';
                            } else if (preg_match('/^(000\.200)/', $result['result']['code'])) {
                                $status = 'pending';
                            }
                        }
